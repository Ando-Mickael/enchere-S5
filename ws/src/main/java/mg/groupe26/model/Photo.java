...
public class Photo {
    ...
    public List<Photo> select(String query, JdbcTemplate jt) {
        return jt.query(query, (rs, row) -> new Photo(rs.getString("id"),
                rs.getString("image"),
                rs.getString("produitid")));
    }

    public void insert(JdbcTemplate jt) {
        String query = String.format("insert into photo values (concat('ADMIN',nexval('id_token'), %s,%s)", getImage(), getProduitid());
        jt.update(query);
    }
